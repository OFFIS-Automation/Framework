...
def _is_generator_function(callableobj):
    return callableobj.__code__.co_flags & 0x20
...
class cached_decorator(object):
    ...
    def __call__(self, callableobj=None):
        ...
        if callableobj.__code__.co_argcount == 1 or self.keyarg == 0:
    ...
    def decorator(func):
        try:
            ...
            raise AttributeError('%s has no __name__ attribute: '
                                 ...
                                 % func)
        if isinstance(func, collections.Callable) and sys.version_info < (3, 0):
