...
def func():
    c = 1
        ...
        # a & b
        a = next(astng.nodes_of_class(nodes.Name))
        ...
        # c
        self.failUnlessRaises(StopIteration, b_infer.__next__)
    ...
    def test_module(self):
        ...
        # built-in objects
        none = next(astng.ilookup('None'))
        self.assertEqual(none.value, None)
        obj = next(astng.ilookup('object'))
        self.assertIsInstance(obj, nodes.Class)
        self.assertEqual(obj.name, 'object')
        self.assertRaises(InferenceError, astng.ilookup('YOAA').__next__)
