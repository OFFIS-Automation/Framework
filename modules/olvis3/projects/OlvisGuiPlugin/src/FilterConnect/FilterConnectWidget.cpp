...
FilterConnectWidget::FilterConnectWidget(const OlvisInterface &model,
                                         QWidget *parent)
    : QDockWidget(parent), mInterface(model), ui(new Ui::FilterConnectWidget)
{
    mCurrentProcessor = 0;
    mPortVisibility = UserPortVisiblility;
    ui->setupUi(this);
    connect(&mInterface, SIGNAL(filterCreated(FilterInfo, int)),
            SLOT(addFilter(FilterInfo, int)));
    connect(&mInterface, SIGNAL(filterDeleted(FilterInfo)),
            SLOT(removeFilter(FilterInfo)));
    connect(&mInterface, SIGNAL(filterMoved(FilterInfo, int)),
            SLOT(moveFilter(FilterInfo, int)));
    connect(&mInterface, SIGNAL(processorCreated(ProcessorInfo)),
            SLOT(addProcessor(ProcessorInfo)));
    connect(&mInterface, SIGNAL(processorDeleted(ProcessorInfo)),
            SLOT(removeProcessor(ProcessorInfo)));
    connect(&mInterface, SIGNAL(processorRenamed(ProcessorInfo, QString)),
            SLOT(renameProcessor(ProcessorInfo)));
    connect(&mInterface, SIGNAL(processorOutputCreated(PortId, QString)),
            SLOT(addProcessorOutput(PortId, QString)));
    connect(&mInterface, SIGNAL(processorOutputRenamed(PortId, QString)),
            SLOT(renameProcessorOutput(PortId, QString)));
    connect(&mInterface, SIGNAL(processorOutputDeleted(PortId)),
            SLOT(removeProcessorOutput(PortId)));
    connect(&mInterface, SIGNAL(filterConnected(PortId, PortId, int)),
            SLOT(addConnection(PortId, PortId, int)));
    connect(&mInterface, SIGNAL(filterDisconnected(PortId, PortId, int)),
            SLOT(removeConnection(PortId, PortId, int)));
    connect(&mInterface, SIGNAL(inputPortConnected(QString, PortId, int)),
            SLOT(addConnection(QString, PortId, int)));
    connect(&mInterface, SIGNAL(inputPortDisconnected(QString, PortId, int)),
            SLOT(removeConnection(QString, PortId, int)));
    connect(&mInterface, SIGNAL(processorsConnected(int, int)),
            SLOT(updateProcessorConnections(int, int)));
    connect(&mInterface, SIGNAL(processorsDisconnected(int, int)),
            SLOT(removeProcessorConnections(int, int)));
    connect(&mInterface, SIGNAL(processorOutputCreated(PortId, QString)),
            SLOT(updateProcessorConnections(PortId)));
    connect(&mInterface, SIGNAL(processorOutputDeleted(PortId)),
            SLOT(updateProcessorConnections(PortId)));
    connect(&mInterface, SIGNAL(processorOutputRenamed(PortId, QString)),
            SLOT(updateProcessorConnections(PortId)));

    connect(&mInterface, SIGNAL(makroFilterCreated(FilterInfo)),
            SLOT(addMakroFilter(FilterInfo)));
    connect(&mInterface, SIGNAL(makroInputCreated(QString, PortId, int)),
            SLOT(addMakroInput(QString, PortId, int)));
    connect(&mInterface, SIGNAL(makroInputRemoved(PortId, int)),
            SLOT(removeMakroInput(PortId, int)));
    connect(&mInterface, SIGNAL(makroFilterDeleted(int)),
            SLOT(removeMakroFilter(int)));

    connect(&mInterface, SIGNAL(filterExecutionError(QString, QString, int)),
            SLOT(showFilterError(QString, QString, int)));
    connect(&mInterface, SIGNAL(filterStartupError(QString, QString, int)),
            SLOT(showFilterError(QString, QString, int)));
    connect(&mInterface, SIGNAL(filterErrorCleared(QString, int)),
            SLOT(removeFilterError(QString, int)));

    connect(this, SIGNAL(newPortValue(int, QString, QVariant)), &mInterface,
            SLOT(setPortValue(int, QString, QVariant)), Qt::QueuedConnection);
    connect(this, SIGNAL(deleteFilterRequested(int)), &mInterface,
            SLOT(deleteFilter(int)), Qt::QueuedConnection);
    connect(this, SIGNAL(addFilterRequested(int, QString)), &mInterface,
            SLOT(addFilter(int, QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(renameFilterRequested(int, QString)), &mInterface,
            SLOT(renameFilter(int, QString)), Qt::QueuedConnection);

    connect(this, SIGNAL(deleteProcessorRequested(int)), &mInterface,
            SLOT(deleteProcessor(int)), Qt::QueuedConnection);
    connect(this, SIGNAL(deleteMakroFilterRequested(int)), &mInterface,
            SLOT(deleteMakroFilter(int)), Qt::QueuedConnection);
}

FilterConnectWidget::~FilterConnectWidget() { delete ui; }
...
void FilterConnectWidget::addProcessorOutput(const PortId &id,
                                             const QString &name)
{
...
void FilterConnectWidget::renameProcessorOutput(const PortId &id,
                                                const QString &name)
{
    FilterInfo filter = mInterface.getFilter(id.filter);
...
void FilterConnectWidget::addConnection(const PortId &src, const PortId &target,
                                        int processorId)
{
    mWidgets[processorId]->addConnection(src, target);
}

void FilterConnectWidget::addConnection(const QString &src,
                                        const PortId &target, int processorId)
{
    mWidgets[processorId]->addConnection(src, target);
}

void FilterConnectWidget::removeConnection(const PortId &src,
                                           const PortId &target,
                                           int processorId)
{
...
void FilterConnectWidget::removeConnection(const QString &src,
                                           const PortId &target,
                                           int processorId)
{
    mWidgets[processorId]->removeConnection(src, target);
...
void FilterConnectWidget::addFilter(const FilterInfo &filterInfo, int beforeFilter)
{
    ...
    connect(w, SIGNAL(newPortValue(int, QString, QVariant)),
            SIGNAL(newPortValue(int, QString, QVariant)));
...
{
    QBoxLayout *layout =
        qobject_cast<QBoxLayout *>(mWidgets[filterInfo.processorId]->layout());
...
void FilterConnectWidget::addMakroInput(QString name, const PortId &portId,
                                        int makroId)
{
...
{
    FilterSortingArea *sortingArea =
        new FilterSortingArea(processor.id, mInterface, false);
    connect(sortingArea, SIGNAL(portSelected(int, QString)),
            SIGNAL(portSelected(int, QString)));
    sortingArea->hide();
...
void FilterConnectWidget::on_deleteProcessoButton_clicked()
{
    ...
    if (gInfo.isValid()) {
        ...
        if (QMessageBox::question(
                0, tr("Delete macro filter"),
                tr("Do you really want to delete the macro filter %1?")
                    .arg(gInfo.name),
                QMessageBox::Yes | QMessageBox::No,
                QMessageBox::No) != QMessageBox::Yes)
            return;
...
void FilterConnectWidget::updateProcessorConnections(const PortId &outputPort)
{
    ...
    if (!filter.isValid())
        return;
    foreach (ProcessingElementConnection connection,
             mInterface.processingElementConnections()) {
        if (connection.sourceId == filter.processorId &&
            mWidgets.contains(connection.targetId))
            mWidgets[connection.targetId]->updateInputs(filter.processorId);
    }
}

void FilterConnectWidget::showFilterError(const QString &filter,
...
void FilterConnectWidget::removeFilterError(const QString &filter,
                                            int /* processorId */)
{
    FilterWidget *widget = mFilterWidgets.value(filter);
    if (widget)
...
void FilterConnectWidget::changeVisibility()
{
    mPortVisibility = UserPortVisiblility;
