...
class CompatTCMixIn:
    ...
    def tearDown(self):
        ...
        for funcname, func in list(self.builtins_backup.items()):
            setattr(builtins, funcname, func)
            # delattr(builtins, 'builtin_%s' % funcname)
        for modname, mod in list(self.modules_backup.items()):
    ...
    def alter_builtins(self):
        for builtin, func in self.ALTERED_BUILTINS.items():
    ...
    def test_removed_modules(self):
        ...
        for modname, funcnames in list(self.MODNAMES.items()):
...
class Py25CompatTC(CompatTCMixIn, TestCase):
    ...
    def test_all(self):
        ...
        self.assertEqual(all(range(10)), False)
        self.assertEqual(all(range(0, -10, -1)), False)
        # python2.5's all consumes iterables
        irange = iter(list(range(10)))
        self.assertEqual(all(irange), False)
        self.assertEqual(next(irange), 1)
