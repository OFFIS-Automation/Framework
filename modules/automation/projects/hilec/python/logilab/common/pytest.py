            ...
            if this_is_a_testfile(filename):
                if self.options.exitfirst and not self.options.restart:
                    ...
                    try:
                        ...
                        print("Error while overwriting \
succeeded test file :", osp.join(os.getcwd(), FILE_RESTART), file=sys.__stderr__)
                        raise e
...
class SkipAwareTextTestRunner(unittest.TextTestRunner):
    ...
    def run(self, test):
        ...
        if not self.batchmode:
            ...
            failed, errored, skipped = list(map(len, (result.failures,
                                                 result.errors,
                                                 result.skipped)))
...
class NonStrictTestLoader(unittest.TestLoader):
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if len(parts) == 1:
            pattern = parts[0]
            if isinstance(getattr(module, pattern, None), collections.Callable)  and pattern not in tests:
