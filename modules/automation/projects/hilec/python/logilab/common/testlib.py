...
class Tags(InheritableSet): # 2.4 compat
    ...
    def __init__(self, *tags, **kwargs):
        ...
        if kwargs:
           raise TypeError("%s are an invalid keyword argument for this function" % list(kwargs.keys()))
    ...
    def __call__(self, result=None, runcondition=None, options=None):
        ...
        try:
            if not self.quiet_run(result, self.setUp):
                return
            generative = is_generator(testMethod.__func__)
            # generative tests
            if generative:
...
succeeded test into", osp.join(os.getcwd(), FILE_RESTART), file=sys.__stderr__)
                        raise ex
    ...
    def assertUnorderedIterableEquals(self, got, expected, msg=None):
        ...
        if len(got) != len(expected):
            if msg is None:
                ...
                for element, count in got_count.items():
        ...
        if any(items.values()):
            if msg is None:
                msg = '\n'.join('%s:\n\t%s' % (key, "\n\t".join(str(value) for value in values))
                    for key, values in items.items() if values)
            self.fail(msg)
...
class MockConfigParser(ConfigParser):
    ...
    def __init__(self, options):
        ...
        for section, pairs in options.items():
