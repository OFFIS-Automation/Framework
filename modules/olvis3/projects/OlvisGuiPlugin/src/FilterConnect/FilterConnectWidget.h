...
public:
    explicit FilterConnectWidget(const OlvisInterface &model,
                                 QWidget *parent = 0);
    ~FilterConnectWidget();
...
signals:
    ...
    void connectRequested(int srcFilter, QString srcPort, int targetFilter,
                          QString targetPort);
...
public slots:
    void addFilter(const FilterInfo &filterInfo, int beforeFilter);
    void addMakroFilter(const FilterInfo &filterInfo);
    ...
    void showFilterError(const QString &filter, const QString &errMsg,
                         int processorId);
