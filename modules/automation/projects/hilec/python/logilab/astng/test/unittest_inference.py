...
class InferenceUtilsTC(TestCase):
    ...
    def test_path_wrapper(self):
        ...
        self.failUnlessEqual(next(infer_end(1)), 1)
...
'''
    ...
    def test_module_inference(self):
        ...
        self.failUnlessRaises(StopIteration, infered.__next__)
    ...
    def test_advanced_tupleassign_name_inference2(self):
        ...
        const = next(infered)
        self.assertIsInstance(const, nodes.Const)
        self.failUnlessEqual(const.value, "glup")
        self.failUnlessRaises(StopIteration, infered.__next__)
...
def f():
    raise NotImplementedError
        '''
        astng = builder.string_build(code, __name__, __file__)
        error = next(astng.nodes_of_class(nodes.Name))
        nie = error.infered()[0]
...
try:
    ...
    raise
        ...
        self.failUnlessRaises(StopIteration, ex1_infer.__next__)
        ex2 = astng['ex2']
        ex2_infer = ex2.infer()
        ex2 = next(ex2_infer)
        self.assertIsInstance(ex2, Instance)
        self.failUnlessEqual(ex2.name, 'Exception')
        self.failUnlessRaises(StopIteration, ex2_infer.__next__)
...
d = a
        ...
        self.failUnlessRaises(StopIteration, n_infer.__next__)
        n = astng['c']
        n_infer = n.infer()
        self.failUnlessRaises(InferenceError, n_infer.__next__)
        n = astng['d']
        n_infer = n.infer()
        infered = next(n_infer)
        self.assertIsInstance(infered, nodes.Const)
...
class A:
    ...
    clsm = classmethod('whatever')
        '''
        astng = builder.string_build(code, __name__, __file__)
        statm = next(astng['A'].igetattr('statm'))
        self.failUnless(statm.callable())
        clsm = next(astng['A'].igetattr('clsm'))
...
def test_view(rql, vid, tags=()):
    ...
    tags.append(vid)
        ...
        self.failUnlessRaises(StopIteration, it.__next__)
...
def f(x):
    ...
    def test_python25_relative_import(self):
        ...
        infered = next(get_name_node(astng, 'date').infer())
    ...
    def test_python25_no_relative_import(self):
        ...
        infered = next(get_name_node(astng, 'import_package_subpackage_module').infer())
