...
FilterSortingArea::FilterSortingArea(int processorId, const OlvisInterface& model, bool allowInputCreation, QWidget *parent) :
    QWidget(parent),
    mInterface(model),
    ...
    ui(new Ui::FilterSortingArea)
{
    ...
    connect(ui->outputs, SIGNAL(startConnect(FilterPortWidget*)), SLOT(onStartConnect(FilterPortWidget*)));
    connect(ui->outputs, SIGNAL(startDisconnect(FilterPortWidget*)), SLOT(onStartDisconnect(FilterPortWidget*)));
    connect(ui->inputs, SIGNAL(startConnect(FilterPortWidget*)), SLOT(onStartConnect(FilterPortWidget*)));
    connect(ui->inputs, SIGNAL(startDisconnect(FilterPortWidget*)), SLOT(onStartDisconnect(FilterPortWidget*)));

    connect(this, SIGNAL(delayedUpdateReq()), SLOT(update()),Qt::QueuedConnection);

    connect(this, SIGNAL(addFilterRequested(int,QString,int)),&mInterface, SLOT(addFilter(int,QString,int)), Qt::QueuedConnection);
    connect(this, SIGNAL(moveFilterRequested(int,int)),&mInterface, SLOT(moveFilter(int,int)), Qt::QueuedConnection);
    connect(this, SIGNAL(connectRequested(int,QString,int,QString)), &mInterface, SLOT(connectFilter(int,QString,int,QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(connectInputRequested(QString,int,QString)), &mInterface, SLOT(connectProcessorInput(QString,int,QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(disconnectRequested(int,QString)), &mInterface, SLOT(disconnectFilter(int,QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(addProcessorOutputRequested(int, QString)), &mInterface, SLOT(createProcessorOutput(int,QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(deleteProcessorOutputRequested(int,QString)), &mInterface, SLOT(deleteProcessorOutput(int,QString)), Qt::QueuedConnection);
}

FilterSortingArea::~FilterSortingArea()
{
    delete ui;
}
...
int FilterSortingArea::dragAnimation(QDropEvent *event)
{
    ...
    {
        FilterWidget* filter = qobject_cast<FilterWidget*>(ui->layout->itemAt(i)->widget());
        if(filter)
        {
...
{
    connect(widget, SIGNAL(startConnect(FilterPortWidget*)), SLOT(onStartConnect(FilterPortWidget*)));
    connect(widget, SIGNAL(startDisconnect(FilterPortWidget*)), SLOT(onStartDisconnect(FilterPortWidget*)));
...
void FilterSortingArea::moveWidget(const FilterInfo &info, int beforeId)
{
    ...
    for(int i=0; i<ui->layout->count(); i++)
    {
        FilterWidget* current = qobject_cast<FilterWidget*>(ui->layout->itemAt(i)->widget());
        if(!current)
            continue;
...
{
    FilterPortWidget* target = ui->outputs->addOutput(portId.filter, portId.port, name);
    // add the connection
    foreach(FilterOutputPortWidget* output, findChildren<FilterOutputPortWidget*>())
    {
        if(output->filterId() == portId.filter && output->portName() == portId.port)
        {
...
{
    FilterPortWidget* target = ui->outputs->takeOutput(portId.filter, portId.port);
    if(!target)
        return;
...
{
    QList<FilterGroupPort> ports = mInterface.processorOutputs(sourceProcessorId);
    QList<ProcessorInputPortWidget*> obsoletePorts = ui->inputs->updatePorts(ports);
...
void FilterSortingArea::removeInputs()
{
    updateInputs(-1);
}
...
void FilterSortingArea::addConnection(const PortId &src, const PortId &target)
{
    ...
    foreach(FilterInputPortWidget* input, findChildren<FilterInputPortWidget*>())
    {
...
void FilterSortingArea::removeConnection(const PortId &src, const PortId &target)
{
...
void FilterSortingArea::addConnection(const QString &src, const PortId &target)
{
    ...
    FilterInputPortWidget* targetWidget = 0;
    foreach(ProcessorInputPortWidget* output, findChildren<ProcessorInputPortWidget*>())
    {
...
void FilterSortingArea::removeConnection(const QString &src, const PortId &target)
{
    foreach(FilterPortWidget* srcWidget, mConnections.keys())
    {
        ProcessorInputPortWidget* input = qobject_cast<ProcessorInputPortWidget*>(srcWidget);
...
void FilterSortingArea::startDrag(FilterWidget* widget, const QPoint &hotspot)
{
    ...
    if(index < 0) index = ui->layout->indexOf(ui->outputs);
    ui->layout->insertWidget(index, mEmptyWidget);
    mEmptyWidget->setVisible(true);
...
void FilterSortingArea::mouseMoveEvent(QMouseEvent *event)
{
    if(!mCurrent.start.isNull())
    {
        ...
        FilterPortWidget* port = firstParentWidget<FilterPortWidget*>(qobject_cast<QWidget*>(child));
...
void FilterSortingArea::mouseReleaseEvent(QMouseEvent *event)
{
    if(!mCurrent.start.isNull())
    {
        ...
        {
            ui->outputs->clearTempPort();
            emit addProcessorOutputRequested(mCurrent.source->filterId(), mCurrent.source->portName());
        }
        else if(port)
        {
            QString warning;
        ...
        {
            ProcessorOutputPortWidget* output = qobject_cast<ProcessorOutputPortWidget*>(mCurrent.ignore);
            if(output)
                emit deleteProcessorOutputRequested(mCurrent.ignore->filterId(), mCurrent.ignore->portName());
            else
                emit disconnectRequested(mCurrent.ignore->filterId(), mCurrent.ignore->portName());
        }
        mCurrent.ignore = 0;
        update();
    }
...
void FilterSortingArea::paintEvent(QPaintEvent *)
{
    ...
    while(connections.hasNext())
    {
        ...
        if(src->isLeft())
            ...
            startPoint = src->mapTo(this, QPoint(src->width(), src->height()/2));
        ...
        while(targets.hasNext())
        {
            ...
            if(!target->isVisible() || mCurrent.ignore == target)
                continue;
        ...
        int layerId = -1;
        QList<LayerUsageList>& layers = src->isLeft() ? leftLayers : rightLayers;
